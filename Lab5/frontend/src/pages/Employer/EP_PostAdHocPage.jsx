...
import styles from './EP_PostInternshipPage.module.css'; // Reuse the same styles
...
const EP_PostAdHocPage = () => {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/jobs/adhoc/my-posts`, {
        headers: {
  ...
  const performDelete = async (id, isDraft = false) => {
    try {
      const token = localStorage.getItem('token');
      const endpoint = isDraft ? 'drafts' : 'adhoc';
  ...
  const handleViewDetails = (id) => {
    navigate(`/employer/adhoc-details/${id}`);
  };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h1>My Ad-Hoc Job Posts</h1>
        <button
          onClick={() => navigate('/employer/add-adhoc')}
          className={styles.addButton}
        >
          <FaPlus /> Add New Ad-Hoc Job
        </button>
      </div>
      ...
      <div className={styles.jobList}>
                  ...
                  <div className={styles.jobDetails}>
                    ...
                    {draft.payPerHour && (
                      <p className={styles.stipendInfo}>
                        <FaDollarSign /> SGD {draft.payPerHour}/hour
                      </p>
                    )}
        ...
        {jobs.length === 0 ? (
          <p className={styles.noJobs}>No published ad-hoc job posts yet.</p>
        ) : (
          <div className={styles.jobGrid}>
            {jobs.map(job => (
              <div key={job._id} className={styles.jobCard}>
                ...
                <div className={styles.jobDetails}>
                  ...
                  <p className={styles.stipendInfo}>
                    <FaDollarSign /> SGD {job.payPerHour}/hour
                  </p>
                  {job.tags?.length > 0 && (
...
export default EP_PostAdHocPage;